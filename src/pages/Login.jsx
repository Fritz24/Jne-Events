...
export default function Login() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setError(err.message || t.failedLogin || "Failed to login. Please check your credentials.");
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex flex-col items-center pt-24 sm:pt-32 pb-12 px-6">
            ...
            >
                <div className="text-center space-y-2 mb-6">
                    ...
                    <p className="text-white/40">{t.loginSubtitle || "Log in to manage your events and bookings"}</p>
                </div>
                ...
                <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-8 backdrop-blur-xl shadow-2xl">
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center ml-1">
                                <label className="text-xs font-semibold text-white/40">{t.passwordLabel || "Password"}</label>
                                <button type="button" className="text-xs font-medium text-violet-400/80 hover:text-violet-300">{t.forgotPassword || "Forgot?"}</button>
                            </div>
                            <div className="relative group">
                        ...
                        >
                            <div className="relative z-10 flex items-center justify-center gap-2">
                                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>{t.signIn || "Sign In"} <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></>}
                            </div>
                        </button>
                    </form>
                    ...
                    <div className="mt-8 pt-8 border-t border-white/5 text-center">
                        <p className="text-white/40 text-sm">
                            ...
                            <Link to="/signup" className="text-violet-400 font-semibold hover:text-violet-300">{t.createOne || "Create one"}</Link>
                        </p>
                    </div>
                </div>
            </motion.div>
